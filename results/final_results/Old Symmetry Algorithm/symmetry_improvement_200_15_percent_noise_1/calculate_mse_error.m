function error_mse = calculate_mse_error(test_image, original_image)
    ...
    for ang=-179:179
        ...
        if norm(rotated_test_image - original_image) < dist_error
            dist_error = norm(rotated_test_image - original_image);
            rot_ang = ang;
            flip_index = 0;
        end
        
        if norm(flipped_image - original_image) < dist_error
            dist_error = norm(flipped_image - original_image);
            rot_ang = ang;
            flip_index = 1;
        end
    end
	...
	error_mse = norm(test_image_registered - original_image);
end