...
% Make sure the image is symmetric in all four quadrants.
flipped_P2 = flip(P, 2);
...
% Check if the image is symmetric about the two axis.
first_quadrant = P(1:(image_size/2), 1:(image_size/2));
second_quadrant = P(1:(image_size/2), (image_size/2) + 1:end);
third_quadrant = P((image_size/2) + 1:end, (image_size/2) + 1:end);
fourth_quadrant = P((image_size/2) + 1:end, 1:(image_size/2));

disp('**** Check if the image is symmetric ****');
disp(norm(first_quadrant - flip(second_quadrant, 2), 'fro'));
disp(norm(second_quadrant - flip(third_quadrant, 1), 'fro'));
disp(norm(third_quadrant - flip(fourth_quadrant, 2), 'fro'));
disp('');
...
addpath(genpath('double_axis_symmetry'));
addpath(genpath('circular_statistics'));
...
if non_uniform_distribution == 0
    filename = ...
        strcat('../results/symmetry/double_axis/', image_name, '/', num2str(sigmaNoiseFraction*100), '_percent_noise/');
else
    filename = ...
        strcat('../results/symmetry/double_axis/', image_name, '/', num2str(sigmaNoiseFraction*100), '_percent_noise/non_uniform_distribution/');
end
...
imwrite(P, strcat(filename,...
    num2str(num_theta), '/original_image.png'));
...
[projections, svector, outlierIndices, allOutlierImages] = ...
    get_projections(theta', percent_outliers, percent_occlusions, P);
...
% Reconstruct the images from projection and store the estimated image.
reconstructed_image = ...
...
if symmetry_prior == 1
    ...
    % The second last parameter is -1 as we don't have any prior
    % for delta yet.
    delta = estimate_double_axis_symmetry_alter(...
        reconstructed_image, output_size, -1, -1, symmetry_method);
end
...
if symmetry_prior == 1
    ...
    for i=1:500
        ...
        symmetry_gradient_vector_1 = symmetry_gradient_matrix(rotated_image);
        symmetry_gradient_vector_1 = imrotate(symmetry_gradient_vector_1, -delta, 'bicubic', 'crop');

        % Rotate the image and calculate the second gradient.
        rotated_image = imrotate(cropped_image, delta + 90, 'bicubic', 'crop');
        symmetry_gradient_vector_2 = symmetry_gradient_matrix(rotated_image);
        symmetry_gradient_vector_2 = imrotate(symmetry_gradient_vector_2, -delta - 90, 'bicubic', 'crop');
        
        % Now finally update the image.
        reconstructed_image = ...
            reconstructed_image - alpha_rate*gradient_vector -...
            beta_rate*symmetry_gradient_vector_1 -...
            beta_rate*symmetry_gradient_vector_2; 

        % The optimization error for this iteration.
        function_error = calculate_optimization_error_symmetry_double_axis(refined_projections,...
    ...
    % Save the image formed sur to symmetric priors.
...
if symmetry_prior == 1
    formatSpec = 'Final symmetric image mse error: %4.2f \r\n';
    fprintf(fileID, formatSpec, calculate_rmse_error(symmetric_reconstructed_image, P));
end
formatSpec = 'Final non-symmetric image mse error: %4.2f \r\n';
fprintf(fileID, formatSpec, calculate_rmse_error(reconstructed_image, P));
